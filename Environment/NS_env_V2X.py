...
class Slidingwindow(object):
    def __init__(self, length, his=3):
        ...
        self.num_CTA = deque(maxlen=length * 350 * 100)
        self.CPA_delay_dist = deque(maxlen=1)  # 协同感知的时延，用于计算QOS
        self.CTA_pls_all = deque(maxlen=100)
        self.SINR_CTA = deque(maxlen=10000)
        self.SINR_CPA = deque(maxlen=6)
        self.complexity = deque(maxlen=1)
        self.complexity_many = deque(maxlen=1000000000)
        self.avg_gain_CP = []
        self.avg_gain_CC = []
        self.avg_gain_CP_CC = []
        self.avg_gain_CC_CP = []
...
class NS_Env:
    def __init__(self, args: Dict[str, Any]) -> None:
        ...
        if self.low_policy == 'CP':
            self.k1 = self.CC_transmission_rate_CP
            self.k2 = self.CP_packet_loss_CP
            self.k3 = self.avg_CP
            self.k4 = self.complexity_CP
            self.beta = 0.05
            self.option = 0
        elif self.low_policy == 'CC':
            ...
            raise ValueError('wrong low policy')
        all_seed()
    ...
    def calculate_slot(self, TTI):
        ...
        for i in range(self.num_of_CPA):
            if self.CPA[i].SC_idx:
                self.qos_CP += self.CPA[i].rate_slot / self.max_rate[i]  # 判断每个用户的速率损失，若不分配信道，则不损失。
            else:
                self.qos_CP += 1

        # print("过程", self.qos_CP,self.max_rate[i])
        self.qos_CP = self.qos_CP * self.w1
        ...
        if TTI == self.max_slot:
            ...
            # x, y = self.get_cdf(self.windows.complexity_many)
            # idx = np.searchsorted(y, 0.8)
            # val_at_80 = x[idx]
            # print("累计概率为 0.8 的数值 =", val_at_80)
        self.qos_complexity = self.complexity / 5 * self.w5
        self.reward = self.qos_CP + self.qos_CC - self.qos_complexity

    def calculate_long_time(self):  # 计算CPA的用户速率，用来求长时间的reward
        packet_loss_all = 0
        ...
        for i in range(self.num_of_CPA):
            ...
            self.CPA[i].rate_all = 0
            self.CPA[i].avg_rate = 0
    ...
    def getState_agent(self, reset=0,step=0):
        self.CPA_rate = []
        if reset == 1:
            state = np.array(np.zeros(self.num_of_CPA))
        else:
            for i in range(self.num_of_CPA):
                self.CPA_rate.append(self.CPA[i].rate_slot)
            state = np.array(self.CPA_rate) / 3000 - 0.7
        # for i in range(self.num_of_CPA):
        #     self.CPA_rate.append(self.CPA[i].rate_slot)

        state = np.hstack((state, self.qos_CP - 0.7))
        state = np.hstack((state, self.complexity / 4 - 0.5))
        state = np.hstack((state, self.au / 0.25 - 0.5))
        state = np.hstack((state, np.sum(self.act_CC_num) / 30 - 0.5))
        state = np.hstack((state, np.sum(self.pls_CC_num) / 5 - 0.5))  # 当前slot的CC丢包数
        state = np.hstack((state, self.windows.avg_gain_CP.flatten()))
        state = np.hstack((state, self.CTA_CSI.flatten() / np.double(5e-7) - 0.5))
        state = np.hstack((state, self.CPA_CTA_CSI.flatten() / np.double(5e-7) - 0.5))
        state = np.hstack((state, self.CTA_CPA_CSI.flatten() / np.double(5e-7) - 0.5))
    ...
    def step_slot(self):
        ...
        self.schedule_CPA()
        self.complexity = 0
        self.act_CC_num = [0 for _ in range(self.CC_num - 2)]
        self.pls_CC_num = [0 for _ in range(self.CC_num - 2)]
